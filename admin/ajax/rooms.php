...
if (isset($_POST['add_room'])) {
    ...
    $q1 = "INSERT INTO `rooms`(`name`, `price`, `adult`, `children`, `description`) VALUES (?,?,?,?,?)";

    $values = [$frm_data['name'], $frm_data['price'], $frm_data['adult'], $frm_data['children'], $frm_data['description']];

    $res = insert($q1, $values, 'siiis');
...
// $q1 = "INSERT INTO `room_features`(`room_id`,`features_id`) VALUES (?,?)";
...
if (isset($_POST['edit_room'])) {
    ...
    $q = "UPDATE `rooms` SET `name`=?, `price`=?, `adult`=?, `children`=?, `description`=? WHERE `id`=?";
    $values = [$frm_data['name'], $frm_data['price'], $frm_data['adult'], $frm_data['children'], $frm_data['description'], $frm_data['room_id']];

    if (update($q, $values, 'siiisi')) {
        echo 1;
    }
}
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo <<<data
            <tr class='align-middle'>
                ...
                <td>$thumb_btn</td>
                <td>
                    <button onclick='rem_image($row[id],$row[room_id])' class='btn btn-warning shadow-none'><i class='bi bi-trash'></i></button
                </td>
            </tr>
        data;
    }
...
if (isset($_POST['remove_image'])) {
    ...
    deletesql("DELETE FROM `room_images` WHERE `room_id`=?", [$frm_data['room_id']], 'i');

    $res2 = deletesql("DELETE FROM `rooms` WHERE `id`=?", [$frm_data['room_id']], 'i');
